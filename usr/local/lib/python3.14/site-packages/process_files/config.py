...
ROOT = 0
COMMENT_REGEX = re_compile(r"#.*$", flags=MULTILINE)
...
def checks_conditions(item: dict) -> bool:
    """Checks if the conditions for an item are met."""
    conditions = item.get("if", {})
    for key, value in conditions.items():
        match key:
            case "hostname":
                if gethostname() != x(value):
                    return False
            case _:
...
def process_config(file: BufferedReader):
    ...
    for folder in data.get("folder", []):
        if checks_conditions(folder):
            folder_item(folder)

    for link in data.get("link", []):
        if checks_conditions(link):
            link_item(link)

    for copy in data.get("copy", []):
        if checks_conditions(copy):
            copy_item(copy)

    for permissions in data.get("permissions", []):
        if checks_conditions(permissions):
            permissions_item(permissions)

    for path in data.get("touch", []):
        if checks_conditions(path):
            touch_item(path)
